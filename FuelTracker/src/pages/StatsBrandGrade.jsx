...
export default function StatsBrandGrade() {
  ...
  const { rows } = useMemo(() => {
    ...
    for (const [, arr] of byVehicle) {
      ...
      let prev = null;
      for (const f of arr) {
        f.unitPrice = Number(f.total_amount) / Number(f.liters); // per L canonical
        if (prev && f.odometer_km > prev.odometer_km) {
          const dist = f.odometer_km - prev.odometer_km;
    ...
    const key = (f) =>
      `${(f.fuel_brand || "").trim().toLowerCase()}|${(f.fuel_grade || "")
        .trim()
        .toLowerCase()}`;
            ...
            {rows.map((r, idx) => {
              ...
              return (
                <tr key={`${r.brand}|${r.grade}|${idx}`} className="border-t">
                  ...
                  <td className="p-2">{priceStr}</td>
                  <td className="p-2">
                    {avgEffDisp != null ? `${num(avgEffDisp, 1)}` : "—"}
                  </td>
